...
import require$$1$4, { app, ipcMain, BrowserWindow } from "electron";
...
function symlinkPaths$1(srcpath, dstpath, callback) {
  if (path$d.isAbsolute(srcpath)) {
    ...
    return pathExists$3(relativeToDst, (err, exists2) => {
      if (err) return callback(err);
      if (exists2) {
        return callback(null, {
          toCwd: relativeToDst,
...
function symlinkPathsSync$1(srcpath, dstpath) {
  let exists2;
  if (path$d.isAbsolute(srcpath)) {
    exists2 = fs$5.existsSync(srcpath);
    ...
    const dstdir = path$d.dirname(dstpath);
    const relativeToDst = path$d.join(dstdir, srcpath);
    exists2 = fs$5.existsSync(relativeToDst);
    if (exists2) {
      return {
        toCwd: relativeToDst,
        toDst: srcpath
...
function createSymlinkSync$1(srcpath, dstpath, type2) {
  ...
  const dir = path$c.dirname(dstpath);
  const exists2 = fs$3.existsSync(dir);
  if (exists2) return fs$3.symlinkSync(srcpath, dstpath, type2);
  mkdirsSync(dir);
...
function parseXml(data) {
  ...
  parser.ontext = (text2) => {
    if (elements.length > 0) {
      elements[elements.length - 1].value = text2;
    }
  };
...
Schema$2.prototype.extend = function extend2(definition) {
  var implicit = [];
  var explicit = [];
...
var int$1 = new Type$7("tag:yaml.org,2002:int", {
  kind: "scalar",
      ...
      if (this.updater.allowPrerelease) {
        ...
        if (currentChannel === null) {
          ...
          for (const element of feed.getElements("entry")) {
            ...
            const hrefChannel = ((_c2 = semver.prerelease(hrefTag)) === null || _c2 === void 0 ? void 0 : _c2[0]) || null;
...
${feedXml}`, "ERR_UPDATER_INVALID_RELEASE_FEED");
    ...
    try {
      ...
      if (this.updater.allowPrerelease && ((_d2 = semver.prerelease(tag)) === null || _d2 === void 0 ? void 0 : _d2[0])) {
        channel = this.getCustomChannelName(String((_e2 = semver.prerelease(tag)) === null || _e2 === void 0 ? void 0 : _e2[0]));
      }
...
class DebUpdater extends BaseUpdater_1$3.BaseUpdater {
  ...
  get installerPath() {
    var _a2, _b2;
    return (_b2 = (_a2 = super.installerPath) === null || _a2 === void 0 ? void 0 : _a2.replace(/ /g, "\\ ")) !== null && _b2 !== void 0 ? _b2 : null;
  }
  doInstall(options) {
...
class SQLiteDialect {
  ...
  buildUpdateSet(table, set2) {
    ...
    const columnNames = Object.keys(tableColumns).filter(
      (colName) => {
        var _a2;
        return set2[colName] !== void 0 || ((_a2 = tableColumns[colName]) == null ? void 0 : _a2.onUpdateFn) !== void 0;
      }
    );
...
const productsTable = sqliteTable("products", {
  ...
  baseUnit: text("base_unit").$type().notNull().default("unit"),
  ...
  updatedAt: text("updated_at").notNull().default(sql`(unixepoch())`),
  deleted: int({ mode: "boolean" }).notNull().default(false)
});
const ProductsRepository = {
  findAll: async () => db.select().from(productsTable).where(eq(productsTable.deleted, false)),
  create: async (data) => db.insert(productsTable).values(data),
  update: async (id, data) => db.update(productsTable).set(data).where(eq(productsTable.id, id)),
  delete: async (id) => db.update(productsTable).set({ deleted: true }).where(eq(productsTable.id, id))
};
...
const registerProductsHandlers = () => {
  ...
  ipcMain.handle("products:remove", async (_, id) => {
...
function readMigrationFiles(config) {
  ...
  if (!fs$j.existsSync(journalPath)) {
    throw new Error(`Can't find meta/_journal.json file`);
  }
  const journalAsString = fs$j.readFileSync(`${migrationFolderTo}/meta/_journal.json`).toString();
  const journal = JSON.parse(journalAsString);
  for (const journalEntry of journal.entries) {
    const migrationPath = `${migrationFolderTo}/${journalEntry.tag}.sql`;
    try {
      const query = fs$j.readFileSync(`${migrationFolderTo}/${journalEntry.tag}.sql`).toString();
...
function runMigrations() {
  const migrationsPath = path$m.resolve(process.cwd(), "drizzle");
  try {
    migrate(db, {
...
const CategoriesRepository = {
  ...
  update: async (id, data) => db.update(categoriesTable).set({ updatedAt: /* @__PURE__ */ new Date(), ...data }).where(eq(categoriesTable.id, id)),
...
const sharp = require$1("sharp");
const registerImagesHandlers = () => {
  ipcMain.handle("save-product-image", async (_, buffer) => {
    const imagesDir = require$$1$1.join(app.getPath("userData"), "products");
    await fs$k.mkdir(imagesDir, { recursive: true });
    const filename = `product-${Date.now()}.webp`;
    const filepath = require$$1$1.join(imagesDir, filename);
    await sharp(buffer).resize(500, 500, { fit: "cover" }).webp({ quality: 100 }).toFile(filepath);
    return { filename };
  });
  ipcMain.handle("get-product-image-path", async (_, filename) => {
    const imagesDir = require$$1$1.join(app.getPath("userData"), "products");
    return require$$1$1.join(imagesDir, filename);
  });
